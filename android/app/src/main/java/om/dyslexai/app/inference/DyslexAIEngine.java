...
public JSObject processAudio(Context context, String audioBase64, String mimeType, String expectedText) throws Exception {
    ...
    String prompt = buildAudioReadingFeedbackPrompt(expectedText);
    Log.i(TAG, "Prompt de áudio/leitura enviada ao runtime:\n" + prompt);
    ...
    String spokenText = safe(parsed.optString("spoken_text", ""), cleanText);

    if (spokenText.trim().isEmpty()) {
        spokenText = safe(expectedText, transcription);
    }
    if (cleanText.trim().isEmpty()) {
        cleanText = spokenText;
    }

    String syllabifiedExpectedText = safe(parsed.optString("syllabified_expected_text", ""), "");
    String syllabifiedSpokenText = safe(parsed.optString("syllabified_spoken_text", ""), "");
    String comparisonSummary = safe(parsed.optString("comparison_summary", ""), "");
    String positiveFeedback = safe(parsed.optString("positive_feedback", ""), "");
    String improvementTip = safe(parsed.optString("improvement_tip", ""), "");
    ...
    meta.put("source", "mobile");
    meta.put("engine", runtime.getName());
    meta.put("mode", "audio-gemma-local-reading-feedback");
...
private String buildAudioReadingFeedbackPrompt(String expectedText) {
    String expected = safe(expectedText, "").trim();
    return "Tarefa: ouvir áudio de uma criança a ler em português europeu e comparar com a frase esperada.\n"
            + "Frase esperada: " + expected + "\n\n"
            + "Devolve APENAS JSON válido, sem Markdown. Usa frases curtas. Não cortes o JSON.\n"
            + "Campos obrigatórios:\n"
            + "{\n"
            + "  \"transcription\": \"o que a criança disse, fiel ao áudio\",\n"
            + "  \"clean_text\": \"transcrição normalizada para mostrar\",\n"
            + "  \"spoken_text\": \"igual a clean_text\",\n"
            + "  \"syllabified_spoken_text\": \"clean_text dividido em sílabas; hífen dentro das palavras e espaços entre palavras\",\n"
            + "  \"comparison_summary\": \"comparação curta com a frase esperada\",\n"
            + "  \"positive_feedback\": \"o que foi bem lido\",\n"
            + "  \"improvement_tip\": \"o que treinar e porquê\",\n"
            + "  \"issues\": [\"erro curto 1\", \"erro curto 2\"]\n"
            + "}\n\n"
            + "Regras para sílabas: exemplo 'O gato comeu o rato.' -> 'O ga-to co-meu o ra-to.'. ";
}
    ...
    result.put("comparison_summary", "Não foi possível obter uma análise completa da leitura.");
    result.put("positive_feedback", "A gravação foi recebida.");
    result.put("improvement_tip", "Tenta gravar novamente num local silencioso.");
